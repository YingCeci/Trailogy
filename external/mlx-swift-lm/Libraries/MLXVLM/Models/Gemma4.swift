...
private func gemma4LogMemory(_ label: String) {
    let snap = Memory.snapshot()
    let mb = 1024.0 * 1024.0
    print(
        String(
            format: "[Gemma4.mem] %@ · active %.0f MB · cache %.0f MB · peak %.0f MB",
            label,
            Double(snap.activeMemory) / mb,
            Double(snap.cacheMemory) / mb,
            Double(snap.peakMemory) / mb
        )
    )
}
...
public final class Gemma4: Module, VLMModel, KVCacheDimensionProvider {
    ...
    private func getInputEmbeddings(
        ...
        var projectedImageFeatures: MLXArray? = nil

        if let pixelValues {
            gemma4LogMemory("vision start")
            var imageFeatures = visionTower(pixelValues)
            eval(imageFeatures)
            gemma4LogMemory("vision tower evaluated")

            imageFeatures = embedVision(imageFeatures)
            eval(imageFeatures)
            gemma4LogMemory("vision projection evaluated")

            projectedImageFeatures = imageFeatures
            Memory.clearCache()
            gemma4LogMemory("vision cache cleared")
        }
        ...
        guard var imageFeatures = projectedImageFeatures else {
            return (inputsEmbeds, perLayerInputs)
        }
        ...
        eval(inputsEmbeds)
        Memory.clearCache()
        gemma4LogMemory("image scatter evaluated")
        ...
        -> PrepareResult
    {
        ...
        if let imagePixels = input.image?.pixels {
            ...
            gemma4LogMemory("language prefill returned")
