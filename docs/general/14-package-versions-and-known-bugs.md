...
This doc supports the Trailogy writeup's **Technical Challenge #1
(Silent PEFT loading failure)**: that bug is one entry in the
"Known bugs" table below; the resolution sequence is in the
"Upgrade order" section.
