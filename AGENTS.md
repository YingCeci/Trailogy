...
Context checkpoint for future agent sessions (Claude Code, OpenCode,
Cursor, Codex). Read this first. `CLAUDE.md` is a symlink to this file
so the same content drives every agent — keep edits here.
...
```
hikeCompanion/
├── README.md, AGENTS.md, CLAUDE.md (symlink → AGENTS.md), ARCHITECTURE.md, project.yml, .gitignore
├── design/
